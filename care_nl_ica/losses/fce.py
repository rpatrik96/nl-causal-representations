...
class ConditionalFCE(object):
    ...
    def __init__(self, data, segments, energy_MLP, flow_model, verbose=False):
        ...
        self.contrastSegments = (
            np.ones(self.segments.shape) / self.segments.shape[1]
        ).astype(np.float32)
        self.energy_MLP = energy_MLP
        self.ebm_norm = -5.0
        self.hidden_dim = self.energy_MLP.linearLast.weight.shape[0]
        self.n_segments = self.segments.shape[1]
        self.ebm_finalLayer = torch.tensor(
            np.ones((self.hidden_dim, self.n_segments)).astype(np.float32)
        )
        # self.ebm_finalLayer = torch.tensor( np.random.random(( self.hidden_dim, self.n_segments )).astype(np.float32) )
        self.flow_model = (
            flow_model  # flow model, must have sample and log density capabilities
        )
    ...
    def compute_ebm_logpdf(self, dat, seg, logNorm, augment=False):
        ...
        if augment:
            ...
            act_allLayer += torch.mm(
                self.energy_MLP(dat) * self.energy_MLP(dat),
    ...
    def train_ebm_fce(
        ...
        if self.verbose:
            print(
                "Training energy based model using FCE" + useVAT * " with VAT penalty"
            )
        ...
        # define
        dat_fce = ContrastiveConditionalDataset(
            np.vstack((self.data, self.noise_samples)).astype(np.float32),
            to_one_hot(y)[0].astype(np.float32),
            np.vstack((self.segments, self.contrastSegments)),
            device=self.device,
        )
        ...
        logNorm = (
            torch.from_numpy(np.array(ebm_norm).astype(np.float32))
            .float()
            .to(self.device)
        )  # , device=dat_fce.device, requires_grad=True )
        ...
        if finalLayerOnly:
            ...
            optimizer = optim.Adam(
                list(self.energy_MLP.parameters()) + [self.ebm_finalLayer] + [logNorm],
                lr=lr,
            )
        ...
        if use_cuda:
            ...
            self.energy_MLP = torch.nn.DataParallel(
                self.energy_MLP, device_ids=range(torch.cuda.device_count())
            )
        ...
        for e in range(epochs):
            ...
            for _, (dat, label, seg) in enumerate(fce_loader):
                ...
                # noise model probs:
                noise_logpdf = self.noise_logpdf(dat).view(
                    -1, 1
                )  # torch.tensor( self.noise_dist.logpdf( dat ).astype(np.float32) ).view(-1,1)
                ...
                # get ebm log pdf
                ebm_logpdf = self.compute_ebm_logpdf(
                    dat, seg, logNorm, augment=augment
                ).view(-1, 1)

                # define logits
                logits = torch.cat(
                    (ebm_logpdf - noise_logpdf, noise_logpdf - ebm_logpdf), 1
                )
            ...
            if self.verbose:
                print(
                    "epoch {} \tloss: {}\taccuracy: {}".format(
                        e, np.round(loss_val, 4), np.round(num_correct / (2 * n), 3)
                    )
                )
            if num_correct / (2 * n) > cutoff:
                ...
                if self.verbose:
                    print(
                        "epoch {}\taccuracy: {}".format(
                            e, np.round(num_correct / (2 * n), 3)
                        )
                    )
    ...
    def pretrain_flow_model(self, epochs=50, lr=1e-4):
        ...
        """
        optimizer = optim.Adam(
            self.flow_model.parameters(), lr=1e-4, weight_decay=1e-5
        )  # todo tune WD
        ...
        if use_cuda:
            ...
            self.flow_model = torch.nn.DataParallel(
                self.flow_model, device_ids=range(torch.cuda.device_count())
            )
            # self.flow_model.to( self.device )
            cudnn.benchmark = True
    ...
    def train_flow_fce(self, epochs=50, lr=1e-4, objConstant=-1.0, cutoff=None):
        ...
        if cutoff is None:
            cutoff = 0.0  # basically only stop for perfect misclassification
        ...
        dat_fce = ContrastiveConditionalDataset(
            np.vstack((self.data, self.noise_samples)).astype(np.float32),
            to_one_hot(y)[0].astype(np.float32),
            np.vstack((self.segments, self.segments)),
            device=self.device,
        )
        fce_loader = DataLoader(dat_fce, shuffle=True, batch_size=128)
        ...
        # define optimizer
        optimizer = optim.Adam(
            self.flow_model.parameters(), lr=lr, weight_decay=1e-5
        )  # todo tune WD
        ...
        for e in range(epochs):
            ...
            for _, (dat, label, seg) in enumerate(fce_loader):
                ...
                # get ebm model probs:
                ebm_logpdf = self.compute_ebm_logpdf(dat, seg, self.ebm_norm).view(
                    -1, 1
                )
            ...
            if self.verbose:
                print(
                    "epoch {} \tloss: {}\taccuracy: {}".format(
                        e, np.round(loss_val, 4), np.round(1 - num_correct / (2 * n), 3)
                    )
                )
            if 1 - num_correct / (2 * n) < cutoff:
                if self.verbose:
                    print(
                        "epoch {}\taccuracy: {}".format(
                            e, np.round(1 - num_correct / (2 * n), 3)
                        )
                    )
                    print("cutoff value satisfied .. stopping training\n----------\n")
                break
    ...
    def unmixSamples(self, data, modelChoice):
        ...
        if modelChoice == "EBM":
            ...
            if self.device == "gpu":
                recov = (
                    self.energy_MLP(torch.tensor(data.astype(np.float32)))
                    .detach()
                    .numpy()
                )
            else:
            ...
            if self.device == "cpu":
                recov = (
                    self.flow_model(torch.tensor(data.astype(np.float32)))[0][-1]
                    .detach()
                    .numpy()
                )
            else:
