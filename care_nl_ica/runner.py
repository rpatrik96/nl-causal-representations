...
class Runner(object):
    ...
    def _dep_mat_metrics(self, dep_mat: torch.Tensor, threshold: float = 1e-3) -> JacobianMetrics:
        ...
        metrics = JacobianMetrics(norm_diff, thresholded_norm_diff, optimal_threshold, sparsity_accuracy,
                                  amari_distance(dep_mat, self.gt_jacobian_decoder_permuted), -1 if self.logger.global_step < 5000 else permutation_loss(extract_permutation_from_jacobian(dep_mat), matrix_exp=True))
