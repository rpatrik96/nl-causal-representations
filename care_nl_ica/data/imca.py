...
class ConditionalDataset(Dataset):
    ...
    def __init__(self, X, Y, device="cpu"):
        ...
        self.y = torch.from_numpy(
            Y
        )  # .to(device)  # if discrete, then this should be one_hot
...
def generateUniformMat_minMax(Ncomp, condT, minVal=0.5, maxVal=1.5):
    """
    generate a random matrix by sampling each element uniformly at random
    check condition number versus a condition threshold
    """
...
def gen_IMCA_data(
    ...
    modMat = (
        np.random.uniform(0.01, 3, (Ncomp, Nsegment)) ** 2
    )  # this is to make consistent with TCL experiments !
    ...
    for i in range(Nsegment):
        ...
        Pres_i = np.linalg.inv(BaseCovariance) * (0.5 / Ncomp) + np.diag(
            1 / modMat[:, i]
        )
        latents[
            (i * NsegmentObs) : ((i + 1) * NsegmentObs), :
        ] = np.random.multivariate_normal(
            mean=np.zeros((Ncomp,)), cov=np.linalg.inv(Pres_i), size=NsegmentObs
        )
...
def generate_synthetic_data(
    ...
    if simulationMethod.lower() == "tcl":
        ...
        baseEvals /= (1.0 / data_dim) * baseEvals.sum()
        baseCov = random_correlation.rvs(baseEvals)

        dat_all = gen_IMCA_data(
            Ncomp=data_dim,
            Nsegment=data_segments,
            Nlayer=n_layer,
            NsegmentObs=n_obs_seg,
