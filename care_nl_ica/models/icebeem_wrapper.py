...
def ICEBEEM_wrapper(
    ...
    prior = TransformedDistribution(
        Uniform(torch.zeros(data_dim), torch.ones(data_dim)), SigmoidTransform().inv
    )
    nfs_flow = NSF_AR
    flows = [
        nfs_flow(dim=data_dim, K=8, B=3, hidden_dim=16) for _ in range(n_layers_flow)
    ]
    ...
    init_ckpt_file = (
        os.path.splitext(ckpt_file)[0] + "_0" + os.path.splitext(ckpt_file)[1]
    )
    if not test:
        if pretrain_flow:
        ...
        fce_.train_ebm_fce(
            epochs=15, augment=augment_ebm, finalLayerOnly=True, cutoff=0.5
        )

        # then train full EBM via NCE with flow contrastive noise:
        fce_.train_ebm_fce(epochs=50, augment=augment_ebm, cutoff=0.5, useVAT=False)
    ...
    for iter_ in range(3):
        mid_ckpt_file = (
            os.path.splitext(ckpt_file)[0]
            + "_"
            + str(iter_ + 1)
            + os.path.splitext(ckpt_file)[1]
        )
        if not test:
            # update flow model:
            fce_.train_flow_fce(
                epochs=5, objConstant=-1.0, cutoff=0.5 - eps, lr=lr_flow
            )
