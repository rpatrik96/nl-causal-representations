...
class EdgeConfidenceLayer(nn.Module):
    ...
    def __init__(
        ...
        self.transform = (
            ...
            else lambda x: torch.clamp(torch.tril(x), 0.0, 1.0)
        )

        self.input_mask_fix = flows.get_mask(
            num_inputs, num_hidden, num_inputs, mask_type="input"
        )
        self.hidden_mask_fix = flows.get_mask(num_hidden, num_hidden, num_inputs)
        self.output_mask_fix = flows.get_mask(
            num_hidden, num_inputs, num_inputs, mask_type="output"
        )

        self.num_ones = (
            self.input_mask_fix.bool().sum()
            + self.hidden_mask_fix.bool().sum()
            + self.output_mask_fix.bool().sum()
        )
    ...
    def input_mask(self):
        return (
            ...
            else self.transform(self.input) * self.input_mask_fix
        )

    def hidden_mask(self):
        return (
            self.hidden_mask_fix
            if self.learnable is False
            else self.transform(self.hidden) * self.hidden_mask_fix
        )

    def output_mask(self):
        return (
            self.output_mask_fix
            if self.learnable is False
            else self.transform(self.output) * self.output_mask_fix
        )
    ...
    def inject_structure(self, adj_mat, inject_structure=False):
        if inject_structure is True:
            ...
            in_rows_selector = [
                hid_rows[i - 1].bitwise_or_(hid_rows[i])
                for i in reversed(range(1, out_rows.sum()))
            ][-1]
...
class AttentionNet(nn.Module):
    ...
    def forward(self, target, attn=None):
        ...
        return target * F.softmax(
            F.linear(
                target if attn is None else attn,
                self.transform(self.attention.weight),
                self.attention.bias,
            ),
            dim=-1,
        )
...
class LearnableMaskedLinear(nn.Module):
    ...
    def forward(self, inputs, cond_inputs=None):
        output = F.linear(
            inputs, self.linear.weight * self.learnable_mask(), self.linear.bias
        )
...
class SplitMADESubnet(nn.Module):
    ...
    def __init__(
        ...
        self.input = LearnableMaskedLinear(
            num_inputs, num_hidden, self.confidence.input_mask, num_cond_inputs
        )
        self.hidden = LearnableMaskedLinear(
            num_hidden, num_hidden, self.confidence.hidden_mask
        )

        if self.num_components is None:
            self.output = LearnableMaskedLinear(
                num_hidden, num_inputs, self.confidence.output_mask
            )
        else:
            self.output = nn.ModuleList(
                [
                    LearnableMaskedLinear(
                        num_hidden, num_inputs, self.confidence.output_mask
                    )
...
class MaskMADE(nn.Module):
    ...
    def __init__(
        ...
        self.s_net = SplitMADESubnet(
            num_inputs, num_hidden, num_components, confidence, act, num_cond_inputs
        )
        self.t_net = SplitMADESubnet(
            num_inputs, num_hidden, num_components, confidence, act, num_cond_inputs
        )
